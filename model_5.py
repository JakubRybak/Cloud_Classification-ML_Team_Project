...
"""Main script for training model #5 (ResNet50 Transfer Learning)."""
...
# from keras.applications import EfficientNetB0, MobileNetV2, DenseNet121 # Not used here
import os
import sys
...
def build_model():
    """Builds the model using ResNet50 as a base for transfer learning."""
    # Create the base ResNet50 model, pre-trained on ImageNet
    # You might need to install keras-applications separately: pip install keras-applications
    base_model = ResNet50(weights='imagenet', include_top=False, input_shape=(config.IMG_WIDTH, config.IMG_HEIGHT, 3))
    ...
    # Corrected comment: Freeze ResNet50 layers, not SE-Net layers
    ...
    print(f"ResNet50 base model loaded. Top {num_layers_to_fine_tune} layers set to trainable.")
    model.summary()
    return model

def main():
    """Main function to run the training and evaluation process."""
    print("Starting training script for ResNet50 model...")
    setup_environment()
