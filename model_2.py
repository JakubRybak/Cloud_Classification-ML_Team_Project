...
def main():
    ...
    evaluation_utils.plot_training_history(history, metrics_to_plot=['accuracy', 'loss', weather_penalty.name, 'precision', 'recall', 'auc'], name='model_2')
    # Perform full evaluation on the validation set
    evaluation_utils.evaluate_model(model, validation_generator, custom_metric_name=weather_penalty.name, name='model_2')
