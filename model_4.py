...
"""Main script for training model #4 (DenseNet121 Transfer Learning)."""
...
# from keras.applications import EfficientNetB0, MobileNetV2 # Not used here
import os
import sys
...
def build_model():
    """Builds the model using DenseNet121 as a base for transfer learning."""
    # Create the base DenseNet121 model, pre-trained on ImageNet
    base_model = DenseNet121(weights='imagenet', include_top=False, input_shape=(config.IMG_WIDTH, config.IMG_HEIGHT, 3))
    ...
    model = Model(inputs=base_model.input, outputs=predictions, name="DenseNet121_Transfer")
    ...
    print(f"DenseNet121 base model loaded. Top {num_layers_to_fine_tune} layers set to trainable.")
    model.summary()
    return model

def main():
    """Main function to run the training and evaluation process."""
    print("Starting training script for DenseNet121 model...")
    setup_environment()
    ...
    evaluation_utils.plot_training_history(history, metrics_to_plot=['accuracy', 'loss', weather_penalty.name, 'precision', 'recall', 'auc'], name='model_4')
    # Perform full evaluation on the validation set
    evaluation_utils.evaluate_model(model, validation_generator, custom_metric_name=weather_penalty.name, name='model_4')

    # Optional: Save the final model
    # final_model_path = "final_densenet_model.keras"
